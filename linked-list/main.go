...
// DLList represents the Doubly Linked List struct
type DLList struct {
	size int
	head *Node
	tail *Node
...
// ToString prints DLList to estout
...
func (l *DLList) Add(el interface{}) {
	if l.IsEmpty() {
...
func (l *DLList) AddFirst(el interface{}) {
	if l.IsEmpty() {
		l.head = &Node{el, nil, nil}
		l.tail = l.head
	} else {
...
func (l *DLList) PeakFirst() (interface{}, error) {
	if l != nil {
		data := l.head.data
		return data, nil
	}
...
func (l *DLList) PeakLast() (interface{}, error) {
	if l != nil {
		data := l.tail.data
		return data, nil
	}
...
func (l *DLList) RemoveFirst() (interface{}, error) {
...
func (l *DLList) RemoveLast() (interface{}, error) {
	// Can't remove data from an empty list
	if l.IsEmpty() {
		err := fmt.Errorf("List is empty")
		return nil, err
...
func (l *DLList) Remove(n *Node) (interface{}, error) {
...
func (l *DLList) RemoveAt(i int) (interface{}, error) {
...
func main() {
	var list DLList
	fmt.Printf("DATA: %v, SIZE: %v, ISCLEAR: %v\n", list.head.ToString(), list.Size(), list.IsEmpty())
	list.Add("HELLO!!!!!!!")
	fmt.Printf("DATA: %v, SIZE: %v, ISCLEAR: %v\n", list.head.ToString(), list.Size(), list.IsEmpty())
	list.Add(22)
	fmt.Printf("DATA: %v, SIZE: %v, ISCLEAR: %v\n", list.head.ToString(), list.Size(), list.IsEmpty())
